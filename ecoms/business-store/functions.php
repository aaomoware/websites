...
function business_store_general_admin_notice(){
	...
	if($pagenow == 'index.php' || $pagenow == 'themes.php'){
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
			 	...
			 	<a href=%9$s target="_blank"  style="text-decoration: none; margin-left:10px;background-color:#d50404;color: white;border: 1px solid #f00e0e;" class="button">%10$s</a>
			 	<a href="?business_store_notice_dismissed" target="_self"  style="text-decoration: none; margin-left:10px;" class="button button-secondary">%11$s</a>
			 	</p></div>',
				...
				esc_html__('Go Pro Version','business-store'),
				esc_url(business_store_THEME_DOC),	
				esc_html__('Tutorial - What is New ?','business-store'),
...
if ( isset( $_GET['business_store_notice_dismissed'] ) ){
	update_option('business_store_notice', 3);
}

$business_store_notice = get_option('business_store_notice', 0);
if($business_store_notice != 3){
 ...
 */
function business_store_featured_area($args, $featured_div = true){
